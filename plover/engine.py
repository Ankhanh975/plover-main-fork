...
class StenoEngine:
    ...
    def __init__(self, config: Any, controller: Any, keyboard_emulation: Any):
        ...
        self._modifier_prev_output = False
    ...
    def _update(self, config_update=None, full=False, reset_machine=False):
        ...
        if reset_machine:
            ...
            # Listen for modifier state changes from the machine (temporary suspend).
            try:
                self._machine.add_modifier_callback(self._on_modifier_state_changed)
            except Exception:
    ...
    def _on_modifier_state_changed(self, modifiers):
        # Ensure handling happens on the engine thread.
        log.info("Modifier state changed: %s", modifiers)
        self._same_thread_hook(self._handle_modifier_change, modifiers)
    ...
    def _handle_modifier_change(self, modifiers):
        """Suspend or resume output based on modifier key set.

        When any modifiers are held, temporarily disable steno output if it
        was previously enabled. When modifiers are released, restore the
        previous output state.
        """
        has_mods = bool(modifiers)
        log.info(
            "_handle_modifier_change called: modifiers=%s, has_mods=%s, disabled=%s, running=%s",
            modifiers,
            has_mods,
            self._modifier_disabled,
            self._is_running,
        )
        if has_mods and not self._modifier_disabled:
            # Modifiers pressed. If engine is running, suspend and remember.
            self._modifier_prev_output = self._is_running
            if self._is_running:
                log.info("Suspending output due to modifiers")
                self._set_output(False)
            self._modifier_disabled = True
        elif not has_mods and self._modifier_disabled:
            # Modifiers released: restore previous state.
            if getattr(self, "_modifier_prev_output", False):
                log.info("Restoring output after modifiers released")
