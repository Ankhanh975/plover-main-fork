...
class KeyboardCaptureProcess(multiprocessing.Process):
    ...
    def _run(self):
        ...
        def on_key(pressed, event):
            # Allow processing of modifier passthrough keys (including Win)
            ...
            if event.vkCode in PASSTHROUGH_KEYS:
                ...
                # Notify main process of modifier state change.
                # Send a special message via the queue: (None, "__MODIFIER_STATE__", tuple_of_vk_codes)
                try:
                    log.info(
                        "Modifier vk change: vk=%s pressed=%s passthrough=%s",
                        event.vkCode,
                        pressed,
                        passthrough_down_keys,
                    )
                    self._queue.put(
                        (None, "__MODIFIER_STATE__", tuple(passthrough_down_keys))
                    )
                except Exception:
                    pass
            ...
            if key is None:
                ...
                return False
            # If modifier(s) are pressed, don't capture this key at all.
            if passthrough_down_keys:
                log.info(
                    "Ignoring key because modifiers held: vk=%s scan=%s key=%s pressed=%s passthrough=%s",
                    event.vkCode,
                    event.scanCode,
                    key,
                    pressed,
...
class KeyboardCapture(Capture):
    def __init__(self):
        ...
        self._thread = None
        self._proc = None
        # Callback to receive modifier state changes: fn(set_of_vk_codes)
    ...
    def _run(self):
        while True:
            ...
            if self._finished.is_set():
                break
            # Special modifier state notification from the subprocess.
            if key == "__MODIFIER_STATE__":
                if self.modifier_callback is not None and isinstance(
                    pressed, (list, tuple, set)
                ):
                    try:
                        self.modifier_callback(set(pressed))
                    except Exception:
                        log.exception("modifier callback failed")
                continue
